...
export default function Atlas({ municipio }: { municipio: Municipio }) {
  return (
    <MunicipalityContext.Provider value={municipio}>
      <MunicipalAtlas key={municipio} />
    </MunicipalityContext.Provider>
  );
}
function MunicipalAtlas() {
  ...
  const c = Object.fromEntries(
    Object.entries(copy[lang]).map(([k, v]) => [
      k,
      v
        .replaceAll('Toa Baja', municipality.name)
        .replaceAll('72137', municipality.geoid)
        .replaceAll('PILOT 01', 'PILOT ' + municipality.pilot),
    ]),
  ) as typeof copy.en;
  useEffect(() => {
      ...
      try {
        const [ml, style, boundary, flood, bld, rd, stats, facilityData] =
          await Promise.all([
            ...
            fetch(municipality.dataPath + '/boundary.geojson', {
              signal: controller.signal,
            }).then((r) => {
              if (!r.ok) throw new Error('Data unavailable');
              return r.json() as Promise<FeatureCollection>;
            }),
            fetch(municipality.dataPath + '/flood.geojson', {
              signal: controller.signal,
            }).then((r) => {
              if (!r.ok) throw new Error('Data unavailable');
              return r.json() as Promise<FeatureCollection>;
            }),
            fetch(municipality.dataPath + '/buildings.geojson', {
              signal: controller.signal,
            }).then((r) => {
              if (!r.ok) throw new Error('Data unavailable');
              return r.json() as Promise<FeatureCollection<Polygon>>;
            }),
            fetch(municipality.dataPath + '/roads.geojson', {
              signal: controller.signal,
            }).then((r) => {
              if (!r.ok) throw new Error('Data unavailable');
              return r.json() as Promise<FeatureCollection>;
            }),
            fetch(municipality.dataPath + '/summary.json', {
              signal: controller.signal,
            }).then((r) => {
              if (!r.ok) throw new Error('Data unavailable');
              return r.json() as Promise<Summary>;
            }),
            fetch(municipality.api('/api/inventory'), {
              signal: controller.signal,
            }).then((r) => {
  ...
  useEffect(() => {
    ...
    register({
      ...
      description:
        'Select the visible municipal FEMA hazard view. This changes map display only; it does not simulate a storm.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    register({
      ...
      execute: () => ({
        ...
        summary,
        scope:
          municipality.name + ' municipal boundary; snapshot coverage only',
      }),
    });
    return () => ac.abort();
  ...
  const download = () => {
    ...
    a.href = url;
    a.download = municipality.slug + '-exposure-summary.json';
    a.click();
  ...
  return (
    <main className={'atlas' + (presenting ? ' presenting' : '')}>
      ...
      <div
        ...
        aria-label={
          lang === 'en'
            ? 'Interactive map of ' + municipality.name
      ...
      <Dialog open={sources} onOpenChange={setSources}>
        <DialogContent className="sources-dialog">
          ...
          <div className="source-body">
            ...
            <h4>{c.next}</h4>
            <p>
              {municipality.slug === 'catano'
                ? lang === 'en'
                  ? 'Cataño includes the two shelters designated in PRDOH’s 2026 list. Coordinates come from the places linked in that list and are not field verified. Police, fire and health points are community-mapped OpenStreetMap records, not field verified. Siren locations remain a gap. Current operation, capacity and safe access remain unconfirmed.'
                  : 'Cataño incluye dos refugios designados por Vivienda en 2026. Las coordenadas provienen de los lugares enlazados en esa lista y no están verificadas en campo. Policía, bomberos y salud incluyen puntos comunitarios de OpenStreetMap, sin verificar en campo. Faltan ubicaciones de sirenas. Operación, capacidad y acceso seguro siguen sin confirmar.'
                : lang === 'en'
